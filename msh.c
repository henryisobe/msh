...
int main()
{
  ...
  while( 1 )
  {
    ...
    // also checks to see if the !n command even exists 
    if(cmd_str[0] == '!')
    {
    ...
    // forks the process and creates a child process
    pid_t pid = fork();
    // child pprocess
    if(pid == 0)
    {
      ...
      //strncpy(token[0], "ls", strlen("ls"));
      //strncpy(token[1], "-l", strlen("-l"));
      
      

      //arguments[2] = NULL;

      // turns child process into an individual one with exec()
